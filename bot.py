...
# ========== НАСТРОЙКИ ==========
BOT_TOKEN = "8882364637:AAHUWNZilUdxotSOXg44owGgCsuozHGlT48"  # ВАШ ТОКЕН
ADMIN_ID = 5611887050  # ЗАМЕНИТЕ НА ВАШ ID
COOLDOWN_DAYS = 7  # Количество дней между заявками
...
async def admin_delete_execute(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Отправляем confirmation админу
    await query.edit_message_text(
    ...
    if apps:
        ...
        keyboard.append([
            ...
            InlineKeyboardButton("💬 Написать",
                                 callback_data=f"admin_reply_to_{current_app['user_id']}_{current_app['id']}")
        ])
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(lambda u, c: admin_view_applications(u, c, None), pattern="^admin_view_all$"))
    app.add_handler(
        CallbackQueryHandler(lambda u, c: admin_view_applications(u, c, "pending"), pattern="^admin_view_pending$"))
    app.add_handler(
        CallbackQueryHandler(lambda u, c: admin_view_applications(u, c, "accepted"), pattern="^admin_view_accepted$"))
    app.add_handler(
        CallbackQueryHandler(lambda u, c: admin_view_applications(u, c, "rejected"), pattern="^admin_view_rejected$"))

    # Пагинация
    app.add_handler(CallbackQueryHandler(lambda u, c: admin_change_page(u, c, "next"), pattern="^admin_next_page$"))
    app.add_handler(CallbackQueryHandler(lambda u, c: admin_change_page(u, c, "prev"), pattern="^admin_prev_page$"))

    # Принятие/отклонение заявок
    app.add_handler(CallbackQueryHandler(lambda u, c: admin_handle_action(u, c, int(c.matches[0].group(1)), "accept"),
                                         pattern=r"^admin_accept_(\d+)$"))
    app.add_handler(CallbackQueryHandler(lambda u, c: admin_handle_action(u, c, int(c.matches[0].group(1)), "reject"),
                                         pattern=r"^admin_reject_(\d+)$"))

    # Удаление заявок
    app.add_handler(CallbackQueryHandler(admin_delete_menu, pattern="^admin_delete_menu$"))
    app.add_handler(CallbackQueryHandler(admin_delete_confirm, pattern=r"^admin_delete_confirm_(\d+)$"))
    app.add_handler(CallbackQueryHandler(admin_delete_execute, pattern=r"^admin_delete_execute_(\d+)$"))

    # Отправка сообщений
    app.add_handler(CallbackQueryHandler(admin_write_user_list, pattern="^admin_write_user$"))
    app.add_handler(CallbackQueryHandler(admin_start_reply, pattern=r"^admin_reply_to_\d+_\d+$"))
